...
@section('content')
    ...
    <header class="bg-linear-to-r from-blue-600 to-blue-800 text-white py-16 px-4 text-center">
        <div class="container mx-auto max-w-6xl">
            <h1 class="text-3xl md:text-4xl font-bold mb-4">Jurusan</h1>
            <p class="text-lg md:text-xl opacity-90">Temukan berbagai jurusan yang kami tawarkan.</p>
        </div>
    </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($jurusans as $jurusan)
                <article class="bg-white rounded-lg shadow-md overflow-hidden group hover:shadow-xl transition-shadow duration-300">
                    @if($jurusan->gambar)
                        ...
                        <img src="{{ '/storage/' . $jurusan->gambar }}" 
                    ...
                    <div class="p-6">
                        <h2 class="text-xl font-bold mb-3 text-gray-900">{{ $jurusan->nama_jurusan }}</h2>
                        
                        @php
                            $desc = trim(strip_tags($jurusan->deskripsi ?? ''));
                        @endphp
                        ...
                        <div class="mt-4 flex justify-between items-center">
                            <button 
                                onclick="window.location.href='{{ $jurusan->slug ? route('jurusan.detail', $jurusan->slug) : '#' }}'"
            ...
            @empty
                <div class="col-span-full text-center py-12">
                    ...
                    <h3 class="text-lg font-medium text-gray-900">Belum ada jurusan</h3>
                    <p class="mt-1 text-gray-500">Jurusan akan segera ditambahkan.</p>
                </div>
            @endforelse
        </div>
